...
def get_new_data_set(interactive=False, numport='4', measure_runs = int(1), fname_postfix='measured', runtimelimit_per_run=5,
	...
	# consider all sensors now
		...
		if interactive:
			print('Make sure to have the calibration cube positioned correctly (axis alignment)')
			while cube == None:
				numport = str(input("Please enter the calibration cube port number X (COM): ")) #4
				if numport == 'quit':
					exit()
				cube = open_port(numport=numport)

				try:
					measure_runs = int(input('Please enter number of measurement runs: '))
				except:
					print('Invalid Input: number of runs must be integer')
				fname_postfix = input('Please enter a name for your output file: ')
		elif cube is None:
			cube = open_port(numport=numport)
			...
			while meas_time < (measure_runs*runtimelimit_per_run):
				try:
					...
					if int(data[0].decode('ascii')) == int(sens_num_to_read):
						if len(data) == 4:
							s_number = int(data[0]) # = sens_num_to_read, else the upper if condition is not met
							field = (meas_time, float(data[1]),float(data[2]),float(data[3]))
					...
					if not on_stage:
						print('Line failed.')
						print('current output from sensor: {}'.format([data[i].decode('ascii') for i in range(len(data))]))
						print(e) 
						...
						f.write('{},{},{},{},{}'.format('Read Failure', 0,0,0,0) +'\r\n')
					else:
						print(e) 
...
if __name__ == "__main__":
	...
	with serial.Serial(port, 256000, timeout=2) as cube: 
		results = get_new_data_set(numport=4, measure_runs = 2000, cube=cube, on_stage=False, no_enter=True)
	duration = time() - t1
	...
	t1 = time()
	for _ in range(5):
		with serial.Serial(port, 256000, timeout=2) as cube: #open serial port; baud rate = 256000
			# results = get_new_data_set(numport=4, specific_sensor=1, measure_runs=int(2), cube=cube)
			# print('sensor 4: {}'.format(results))

			results = get_new_data_set(numport=4, specific_sensor=64, measure_runs=int(2), cube=cube)
